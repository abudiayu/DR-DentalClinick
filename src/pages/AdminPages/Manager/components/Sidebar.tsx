...
export default function Sidebar({ active, onChange, open, onClose }: SidebarProps) {
  ...
  const items: { key: SidebarSection; label: string; icon: React.ReactNode }[] = [
    { key: 'dashboard',    label: t('manager.dashboard'),    icon: <LayoutDashboard className="w-4 h-4" /> },
    { key: 'patients',     label: t('manager.patients'),     icon: <Users           className="w-4 h-4" /> },
    { key: 'payments',     label: t('manager.payments'),     icon: <CreditCard      className="w-4 h-4" /> },
    { key: 'appointments', label: t('manager.appointments'), icon: <Calendar        className="w-4 h-4" /> },
    { key: 'services',     label: t('manager.services'),     icon: <Stethoscope     className="w-4 h-4" /> },
    { key: 'reports',      label: t('manager.reports'),      icon: <BarChart3       className="w-4 h-4" /> },
    { key: 'expenses',     label: t('manager.expenses'),     icon: <Receipt         className="w-4 h-4" /> },
    { key: 'settings',     label: t('manager.settings'),     icon: <Settings        className="w-4 h-4" /> },
  ]
    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center px-1 py-2 border-b border-white/10">
          ...
          <button onClick={onClose} className="lg:hidden text-white/40 hover:text-white ms-auto">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {items.map(item => (
            ...
            >
              ...
              {active === item.key && (
                <span className="ms-auto w-1 h-4 rounded-full bg-indigo-400" />
              )}
            </motion.button>
          ))}
        </nav>
