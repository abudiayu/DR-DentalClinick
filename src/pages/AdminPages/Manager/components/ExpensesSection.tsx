    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className="text-xl font-bold text-indigo-600">{totalIncome.toLocaleString()} {t('common.birr')}</p>
          <p className="text-[11px] text-slate-400 uppercase tracking-widest mt-0.5">{t('manager.totalIncome')}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className="text-xl font-bold text-red-500">{totalExpenses.toLocaleString()} {t('common.birr')}</p>
          <p className="text-[11px] text-slate-400 uppercase tracking-widest mt-0.5">{t('manager.totalExpenses')}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className={`text-xl font-bold ${netProfit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
            {netProfit.toLocaleString()} {t('common.birr')}
          </p>
          <p className="text-[11px] text-slate-400 uppercase tracking-widest mt-0.5">{t('manager.netProfit')}</p>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        ...
        >
          <Plus className="w-3.5 h-3.5" /> {t('manager.addExpense')}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
          <h3 className="text-xs font-semibold text-slate-600 uppercase tracking-widest">{t('manager.newExpense')}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">{t('manager.title')}</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">{t('manager.amount')}</label>
              <input required type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-slate-400" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">{t('manager.category')}</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">{t('manager.date')}</label>
              <input required type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-slate-400" />
            </div>
          </div>
          <div className="flex gap-2 pt-1">
            <button type="submit" className="bg-[#0F172A] text-white text-xs px-4 py-2 rounded-lg hover:bg-slate-700 transition-colors">{t('common.save')}</button>
            <button type="button" onClick={() => setShowForm(false)} className="text-xs px-4 py-2 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 transition-colors">{t('common.cancel')}</button>
          </div>
        </form>
      )}
