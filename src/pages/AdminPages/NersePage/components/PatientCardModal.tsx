...
interface Props { patient: NursePatient; onClose: () => void }
...
export default function PatientCardModal({ patient, onClose }: Props) {
  ...
  const qrPayload = JSON.stringify({
    id: patient.id, card: patient.cardNumber,
    name: patient.fullName, date: patient.visitDate,
  ...
  function handlePrint() {
    ...
    win.document.write(`
      ...
      <style>
        ...
        .qr { margin-top: 12px; } img { width: 80px; height: 80px; }
      </style></head><body>
      <div class="card">
  ...
  const detailRows: [string, string][] = [
    [t('nurse.age2'),             `${patient.age} · ${patient.gender}`],
    [t('nurse.phoneNumber'),      patient.phone],
    [t('nurse.serviceType'),      patient.serviceType],
    [t('nurse.visitDate'),        patient.visitDate],
    [t('nurse.cardFee'),          `${patient.cardFee} ${t('common.birr')}`],
  ]
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div>
              <p className="text-sm font-bold text-[#0d2044] uppercase tracking-widest">{t('nurse.patientCard')}</p>
              <p className="text-[11px] text-slate-400 mt-0.5">{t('nurse.generatedSuccessfully')}</p>
            </div>
            <button onClick={onClose} className="text-slate-400 hover:text-slate-700 transition-colors" aria-label={t('common.close')}>
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            <div ref={cardRef} className="rounded-2xl overflow-hidden"
              ...
              <div className="p-5 flex gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-cyan-400 text-xs font-mono font-bold tracking-widest mb-1">{patient.cardNumber}</p>
                  <p className="text-white text-lg font-bold leading-tight mb-3 truncate">{patient.fullName}</p>
                ...
                <div className="flex flex-col items-center gap-1 flex-shrink-0">
                  ...
                  <p className="text-[9px] text-white/30 text-center">{t('nurse.scanToVerify')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="px-5 pb-5 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {emailState === 'sent' ? t('nurse.emailSent') : t('nurse.sendEmail')}
              </motion.button>
              ...
              >
                ...
                {telegramState === 'sent' ? t('nurse.telegramSent') : t('nurse.telegram')}
              </motion.button>
            </div>
